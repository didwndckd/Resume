...
- 프로젝트 규모 증가로 빌드 타임이 길어져 SwiftUI 프리뷰 사용이 불가능해진 문제 해결 및 의존성 관리를 위해 도입
- Domain, Data 레이어 분리로 비즈니스 로직과 데이터 계층 격리
- Feature, Foundation, UIFoundation, DesignSystem 등 역할 기반 모듈 분리
- 전체 코드베이스의 24% 모듈 분리 완료 및 단계적 전환 진행 중
...
#### 기술 스택 현대화
- UIKit/Storyboard 기반 화면을 SwiftUI로 단계적 전환, 전환율 52.6% 달성
- Combine/Completion Handler 기반 비동기 처리를 Swift Concurrency로 전환, 전환율 70.7% 달성
