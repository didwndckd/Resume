...
MAU 300만 규모의 크몽에서 iOS 개발자로 근무하며 앱의 구조적 안정성과 확장성을 개선하는 작업을 주도해왔습니다.

Tuist 기반 모듈화 설계, 실시간 채팅 아키텍처 개선, 디자인 시스템 구축 등을 통해 대규모 코드베이스의 유지보수성과 개발 생산성을 높이는 데 기여했습니다.

Clean Architecture와 MVVM을 기반으로 개발하며, Combine을 활용한 리액티브 프로그래밍과 구조적 문제 해결에 강점이 있습니다.
...
(주)크몽 | 2023.07.13 ~ 재직중
프리랜서/아웃소싱 플랫폼 (MAU 300만, 누적 거래 640만 건 이상) / iOS 팀 4명 / Swift 약 36만 LOC

• 모듈화 구조 설계 및 도입 (Tuist)
  - 앱 규모 증가로 인한 의존성 복잡도와 빌드 관리 문제를 해결하기 위해 도입
...
• 실시간 채팅(Pusher WebSocket) 아키텍처 개선
...
• Endpoint 패턴 도입으로 API 관리 구조 개선
  - 기존 Moya TargetType enum 방식에서 API별 개별 Endpoint struct로 전환
  - associatedtype을 활용하여 API와 ResponseModel의 1:1 매칭을 컴파일 타임에 검증하도록 설계
  - 요청 정보가 하나의 struct에 응집되어 API 히스토리 파악 용이
...
• 전체 화면 메모리 누수 제거
  ...
  - WKUserContentController의 강한 참조 문제를 프록시 패턴으로 해결

[제주패스] 항공, 숙박, 렌터카 가격 비교/예약 서비스
...
RoboRisen(외주 프로젝트) | 2021.10.21 ~ 2021.12.29

[코딩로비] 블루투스 연결을 통해 로봇을 제어하며 놀 수 있는 아이들을 위한 코딩 교육 앱

시연 영상: https://www.youtube.com/watch?v=2jRoFh67k18

• RxSwift, MVVM 패턴, 스토리보드 사용
• CoreBluetooth를 사용하여 큐브 연결과 데이터 입출력 제어
• 블루투스 데이터 전송 간 Queue를 구현하여 데이터를 안전하게 전송할 수 있도록 구현
...
AngelHack Seoul 2020 | 2020.07.13 ~ 2020.07.19
...
JCJoystick | 2022.04.14 ~ 2022.04.30
...
Netflix Clone | 2020.03.20 ~ 2020.04.29
...
담당 작업: 영상 재생, 콘텐츠 저장, 로그인/회원가입, 통신 모듈


[학력]

국가평생교육진흥원 학점은행 | 2019.09 ~ 2023.08
• 컴퓨터공학사 (4.1/4.5)
