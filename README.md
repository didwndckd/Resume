...
MAU 300만 규모의 크몽에서 iOS 개발자로 근무하며, 모듈화 설계, 채팅 아키텍처 개선, 디자인 시스템 구축 등 앱의 구조적 기반을 주도적으로 개선하고 있습니다.

Clean Architecture와 MVVM을 기반으로 개발하며, Tuist를 활용한 모듈화 구조 설계와 Combine 기반 리액티브 프로그래밍에 강점이 있습니다.
...
프리랜서/아웃소싱 플랫폼 (MAU 300만, 누적 거래 640만 건 이상) / iOS 팀 4명

![Kmong_preview](Assets/Kmong_preview.png)

- 모듈화 구조 설계 및 도입 (Tuist)
  - 앱 모듈 비대화로 SwiftUI Preview 불가 문제를 해결하기 위해 도입
  - Domain, Data 레이어 분리로 비즈니스 로직과 데이터 계층 격리
  - Feature, Foundation, UIFoundation, DesignSystem 등 역할별 모듈 분리
...
- 실시간 채팅(Pusher WebSocket) 아키텍처 개선
  ...
  - `DiffableDataSource` 도입 및 비활성 상태 이벤트 버퍼링으로 메시지 UI 업데이트 안정화
  - 인덱스 불일치로 인한 채팅 화면 크래시 **0건**으로 개선
...
- Endpoint 패턴 도입으로 API 관리 구조 개선
  ...
  - API-ResponseModel 1:1 매칭을 associatedtype으로 컴파일 타임에 강제
...
원하는 곳에서 렌터카를 픽업하고 반납할 수 있는 렌터카 서비스 / iOS 팀 2명
...
- `Moya.PluginType`을 사용한 네트워크 통신 플러그인 개발
  ...
  - API 응답 시 서버에서 갱신된  `AccessToken`과 `RefreshToken`이 헤더에 들어오게 되면 앱 내의 토큰을 갱신하는 플러그인 개발
- FCM 푸시 처리 모듈 개발
